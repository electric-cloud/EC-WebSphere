...
class StopApplicationServersNegative extends PluginTestHelper {
    ...
    @Shared
    def testProcedureName =         'StopApplicationServers'
    ...
    */
    ...
    def exs = [
        ...
        wrongServer: "Failed to stop server $serverNames.incorrect on node $nodeNames.correct",
        wrongNode: "Failed to stop server $serverNames.correct on node $nodeNames.incorrect",
        wrongBoth: "Failed to stop server $serverNames.incorrect on node $nodeNames.incorrect",
     ...
     */
     ...
     def doSetupSpec() {
        ...
        // actual test project
        importProject(testProjectName, 'dsl/StopApplicationServers/Procedure.dsl', [projectName: testProjectName, wasResourceName:wasResourceName])
    ...
    @Unroll
    def "StopApplicationServers. Negative scenario." () {
        when: 'Procedure runs: '
        def runParams = [
            wasHost:    wasHost,
            configName: cfgName,
        ...
        def jobSummary = getJobProperty('/myJob/jobSteps/StopApplicationServers/summary', result.jobId)
