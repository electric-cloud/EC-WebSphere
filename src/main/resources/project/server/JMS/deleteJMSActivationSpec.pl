...
my $opts = {
    ...
    activationSpecScope              => '$[activationSpecScope]',
    activationSpecAdministrativeName => '$[activationSpecAdministrativeName]',
};
...
my $parsedActivationSpecScope = $websphere->parseScope($opts->{activationSpecScope});
$websphere->setTemplateProperties(
    wasApi     => $wasApi,
    activationSpecScope => $parsedActivationSpecScope,
);

my $logger = $websphere->log();
my $file = 'delete_jms_activation_spec.py';
...
$props{DeleteJMSActivationSpecLine} = $escaped_shellcmd;
...
my $result_params = {
    ...
    pipeline => {
        target => 'Delete JMS ActivationSpec Result:',
        msg => '',
    }
};
...
if ($code == SUCCESS) {
    my $message = sprintf 'Successfully deleted %s: %s for %s scope',
        $wasApi,
        $opts->{activationSpecAdministrativeName},
        $parsedActivationSpecScope;

    if ($cmd_res =~ m/Status:\sOK,\sMessage:\s(.*?)$/ms) {
        $message = $1;
        $message .= " for $parsedActivationSpecScope scope";
    }
    ...
    if ($error) {
        $error = "Error occured during $operation_mode of $wasApi $opts->{activationSpecAdministrativeName}:\n$error";
    }
    else {
