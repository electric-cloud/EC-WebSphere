...
my $parsedConnectionFactoryScope = $websphere->parseScope($opts->{factoryScope});
$websphere->setTemplateProperties(
    wasApi     => $wasApi,
    factoryScope => $parsedConnectionFactoryScope,
);

my $logger = $websphere->log();
my $file = 'delete_jms_connection_factory.py';
...
$props{DeleteJMSConnectionFactoryLine} = $escaped_shellcmd;
...
my $result_params = {
    ...
    pipeline => {
        target => 'Delete JMS Connection Factory Result:',
        msg => '',
    }
};
...
if ($code == SUCCESS) {
    my $message = sprintf 'Successfully deleted %s: %s for %s scope',
        $wasApi,
        $opts->{factoryAdministrativeName},
        $parsedConnectionFactoryScope;

    if ($cmd_res =~ m/Status:\sOK,\sMessage:\s(.*?)$/ms) {
        $message = $1;
        $message .= " for $parsedConnectionFactoryScope scope";
    }
    ...
    if ($error) {
        $error = "Error occured during $operation_mode of $wasApi $opts->{factoryAdministrativeName}:\n$error";
    }
    else {
