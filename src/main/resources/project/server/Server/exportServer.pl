...
$websphere->setTemplateProperties(
    exportParams  => $r->render()
);

my $logger = $websphere->log();
my $file = 'export_server.py';
...
$props{exportServerLine} = $escaped_shellcmd;
...
if ($code == SUCCESS) {
    ...
    $result_params->{procedure}->{msg} = sprintf(
        'Server %s on node %s has been exported to %s',
        $app_server_name, $node_name, $archive_path
    );
    ...
    $result_params->{procedure}->{msg} = $result_params->{pipeline}->{msg} =
        sprintf(
            'Failed to export server %s on node %s to %s.' . "\n" . 'Error: %s',
            $app_server_name,
            $node_name,
            $archive_path,
            $error
