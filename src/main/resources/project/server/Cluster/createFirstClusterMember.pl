...
my $step_params = {
    target => {
        pipeline => 'Create First cluster member Result:',
        success_summary => 'First cluster member has been created.',
        error_summary   => 'Failed to create a first cluster member.'
    },
    jython_script => {
        path => 'create_first_cluster_member.py',
    },
};
