...
my $step_params = {
    target => {
        pipeline => 'Remove Cluster Members result:',
        success_summary => 'Cluster members have been removed.',
        error_summary   => 'Failed to remove cluster members.'
    },
    jython_script => {
        path => 'remove_cluster_members.py',
    },
};
